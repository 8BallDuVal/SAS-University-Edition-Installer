from tkinter import Tk, WORD, Frame, PhotoImage, Label, Button, RIGHT, LEFT, CENTER, N, S, W, E, END, Text, SUNKEN, messagebox, DISABLED, filedialog, BOTH
import subprocess
import webbrowser
...
class Installer(Frame):
    ...
    def initUI(self):
        ...
        analyticsphoto = PhotoImage(data=base64_image)
        self.analytics_u_photo = Label(self, image=analyticsphoto, justify=CENTER)
...
#----- Step 1: Check if VirtualBox is installed -----#
    ...
    def first_action_button_clicked(self):
        if self.check_vbox_installation() == True:
            ...
            self.messagelabel.config(text=" Click the 'Continue' button to check to see if 64-bit Hardware Virtualization\n"+
            ...
            self.messagelabel.config(text="\t\t\tDownload Oracle VirtualBox", foreground="blue", cursor="hand2")
...
#----- Step 2: Check if Virtualization is enabled -----#
    ...
    def second_action_button_clicked(self):
        ...
        if self.confirm_vtx_enabled() == True:
            ...
            messagebox.showerror("Virtualization is disabled"," Virtualization is disabled on your system."+
                                                              " You will need to contact your PC manufacturer "+
                                                              "(Dell, Acer, Lenovo) for instructions on how to enable 64-bit "+
                                                              " Hardware Virtualization.\n\n") 
            self.messagelabel.config(text="\t\t\tSAS Note: Virtualization is Disabled", fg="blue", cursor="hand2")
...
#----- Step 3: Set up Shared folder (either the default directory or user-selected) -----#
    ...
    def third_action_button_clicked(self):
        self.messagelabel.config(text=    " Either choose an existing 'myfolders' folder on your hard drive, or a 'myfolders' \n"+
...
#----- Step 4: Locate vApp (auto-searches the Downloads folder. If not found, user must search for it) -----#    
    def locate_vapp(self):
        ...
        self.messagelabel.config(text=    " Click the 'Continue' button to scan your hard drive for the SAS University Edition \n"+
    ...
    def fourth_action_button_clicked(self):
        ...
        if list_of_files != []:
            ...
            self.action_button.config(state=DISABLED)
            self.messagelabel.config(text="\t\t\tDownload SAS University Edition vApp", foreground="blue", cursor="hand2")
...
#              folders in the 'VirtualBox VMs' folder. -----#  
    def fifth_action_button_clicked(self):
        self.choose_sharedfolder.grid_remove()
        self.action_button.config(state='normal', command=self.check_duplicates)
        self.messagelabel.destroy()
        self.messagelabel = Label(self, text=" Click the 'Continue' button to verify that you do not have any duplicate SAS\n"+
...
#----- Step 6: Write batch file using information collected above & run batch file to import vApp -----#   
    def sixth_action_button_clicked(self):
        ...
        self.messagelabel.config(text=" Click the 'Import' button to import the SAS University Edition vApp\n "+
